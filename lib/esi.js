        ...
        tags.forEach(tag => {
            ...
            if(maxDepthReached) {
                ...
                subtasks[i] = getIncludeContents(tag, options)
                    .then(result => html = html.replace(placeholder, result.body+'<span id="'+result.esipath+'" name="esi-timing" class="esi-timing" style="display:none" value="'+result.timing+'">'+result.timing+'</span>'));
                i++;
            }
        });
